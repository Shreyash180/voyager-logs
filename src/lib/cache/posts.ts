...
export function postListCacheKey(params: { page: number; limit: number; q?: string; tag?: string }) {
  const q = params.q ?? "";
  const tag = params.tag ?? "";
  return `${POST_LIST_CACHE_PREFIX}p=${params.page}&l=${params.limit}&q=${q}&tag=${tag}`;
}
